  ...
  return (
    <main>
      <PageHeader
        kicker="Bitcoin"
        title="BTC on-chain"
        description={`${planLabel(payment.plan)} · ${formatUsd(payment.amount_usd) ?? plan?.priceLabel} USD. Send Bitcoin only on the Bitcoin network.`}
        action={
          <Link
      ...
      <div className="mx-auto max-w-3xl px-6 py-14 lg:px-10">
        <PaymentPanel payment={payment} method="btc" />
      </div>
    </main>
  );
}
